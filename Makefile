...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"shift_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f shift-*.tar.gz
	echo Create archive \"shift-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../shift-$(VERSION)-$(BRANCH).tar.gz shift
...
shift.tar.gz: build
	cd make_build; tar czf ../shift.tar.gz shift && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./shift make_build
	cp *.md make_build/shift/
